...
#### Принцип действия
Программа под целевой (target) микроконтроллер (МК) пишется, собирается на хост машине (Windows, Linux, MacOS). Затем готовый hex файл с помощью программы avrdude, запускаемой на хост машине, по виртуальному COM порту (реализован поверх USB) напрямую прошивается в целевой МК, подключенный к модулю по SPI. Используется специальный режим микросхемы FTDI "bitbang" (8 разрядный порт чипа FT2232 + специально заточенный драйвер FTDI позволяют программно управлять периферией с хоста, в частности, писать команды и данные в МК через SPI)
